...
componentDidMount=()=>{
  ...
  axios.get('https://data.sfgov.org/resource/gxxq-x39z.json?pddistrict=MISSION&dayofweek=Friday&$order=date%20DESC&$where=date%3E%272017-01-01T12:00:00%27&category=VANDALISM]').then(function(response){
  ...
  return(
    <div>
        ...
        {this.state.crime.map(function(stat, i){
          return(
            <p key={i}>{stat.address} {stat.date} {stat.dayofweek}</p>
          )
        })}
    </div>
  )
